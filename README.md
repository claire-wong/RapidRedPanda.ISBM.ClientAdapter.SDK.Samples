...
## Contents
   ...
   2. [ISBM Client Adapter Architecture](#ISBM-Client-Adapter-Architecture-Diagram)
...
Since OIIE involves multiple industry standards at different levels of the technology stack, it is helpful to have developer friendly OIIE programming interfaces. This SDK includes sample C# code projects that consume the RapidRedPanda.ISBM.ClientAdapter 2.2.0 NuGet package. The package includes assets for multiple .NET target frameworks, while the current public samples target Windows WinForms on .NET Framework 4.7.2 and Raspberry Pi OS console applications on .NET 8; see details in the ISBM Client Adapter Architecture Diagram. By using the adapter, you can deliver and receive CCOM messages (Common Conceptual Object Model) through ISBM services (ISA-95 Message Service Model) with just a few lines of code! It reduces the learning curve, allowing you to explore OIIE without diving into the technical details of the standards
...
#### ISBM Client Adapter Architecture Diagram

![ISBM Client Adapter Architecture](Documents/Images/Architecture_Diagram1.svg)
